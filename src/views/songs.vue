        ...
        <div class="flex flex-col space-y-4 w-full h-full p-3">
          ...
          <div class="flex w-full py-4 overflow-auto scrollBar p-2">
            <div class="flex flex-col w-full space-y-1">
              <div v-for="(track, index) in tracksList" 
                   :key="track.id || index"
                   @click="playTrack(index)" 
                   :class="`w-full ${index%2 == 0 ? `bg-white/10` : `bg-transparent`} ${isActiveTrack(track) ? 'bg-cyan-500/20 border border-cyan-500/30' : ''} p-1 px-2 rounded-md cursor-pointer hover:bg-white/20 transition-colors`">
                  
                  <div v-if="track != undefined" class="flex space-x-2 items-center justify-between text-[0.70em] font-light">
                      <div class="flex w-[25%] space-x-2 items-center justify-start text-truncate">
                          <p class="flex flex-shrink-0 w-8 justify-center">
                            {{ isActiveTrack(track) && !isPlaying ? '⏸️' : index + 1 }}
                          </p>
                          
                          <div class="flex flex-shrink-0 w-10 h-10 bg-black rounded-md overflow-hidden">
                      ...
                      <div class="flex w-[12.5%] justify-end items-center">
                          <button @click="(e)=>{e.stopPropagation(), showTrackOptions(track)}" class="p-1 hover:bg-white/10 rounded transition-colors">
...
const playTrack = (index) => {
  ...
  if (isActiveTrack(tracksList.value[index])) {
    musicStore.togglePlay();
  } else {
    // Sinon on joue la nouvelle musique
    musicStore.playTrack(tracksList.value, index);
  }
};

// Ordonner les musiques
...
// Afficher les options d'une musique
const showTrackOptions = (track) => {
  console.log('Options pour:', track);
  // Ici vous pouvez implémenter un menu contextuel
  // Exemple : ajouter à une playlist, voir les détails, etc.
};

// Fermer les options quand on clique ailleurs
const handleClickOutside = (event) => {
  if (showOrderOptions.value && !event.target.closest('.relative')) {
    showOrderOptions.value = false;
  }
};
